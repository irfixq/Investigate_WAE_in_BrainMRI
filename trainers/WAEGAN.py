...
class WAEGAN(AEMODEL):
    class Config(AEMODEL.Config):
        def __init__(self):
            super().__init__('WAEGAN')
            self.scale = 10.0
            self.kappa = 1.0
    ...
    def train(self, dataset):
        ...
        for epoch in range(last_epoch, self.config.numEpochs):
            ...
            for idx in range(0, num_batches):
                ...
                # Print to console
                print(f'Epoch ({phase.value} WAEGAN): [{epoch:2d}] [{idx:4d}/{num_batches:4d}]'
            ...
            self.log_to_tensorboard(epoch, scalars, visuals, phase, name='waegan_x')
    ...
    def reconstruct(self, x, dropout=False):
        if x.ndim < 4:
            x = np.expand_dims(x, 0)  ## can also use np.squeeze
